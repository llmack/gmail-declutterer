            ...
            <>
              <nav className="flex space-x-2 mr-4">
                <Link href="/dashboard" onClick={(e) => {
                  // This ensures we go to the overview tab when clicking Dashboard in the header
                  const dashboardEl = document.getElementById('dashboard-component');
                  if (dashboardEl) {
                    e.preventDefault();
                    // If we're already on the dashboard page, set active category to overview
                    const event = new CustomEvent('set-overview', { detail: 'overview' });
                    dashboardEl.dispatchEvent(event);
                  }
                }} className="px-3 py-2 text-gray-700 hover:text-gray-900 hover:bg-[#8bbdd6] rounded-md transition-colors flex items-center">
